...
functions.http('send', async (req, res) => {
  try {
    ...
    const cover = await client.uploads.find(
...
function getEmailSubject(req) {
  switch (true) {
    case req.body.entity.attributes.language === 'ita' &&
      req.body.entity.attributes.blog_type === 'dev':
      return 'Nuovo articolo'
    case req.body.entity.attributes.language === 'eng' &&
      req.body.entity.attributes.blog_type === 'dev':
      return 'New Article'
    case req.body.entity.attributes.language === 'ita' &&
      req.body.entity.attributes.blog_type === 'finance':
      return 'Nuovo articolo'
    case req.body.entity.attributes.language === 'eng' &&
      req.body.entity.attributes.blog_type === 'finance':
      return 'New Article'
...
function getEmailTemplate(req, cover, user) {
  switch (true) {
    ...
    case req.body.entity.attributes.language === 'ita' &&
      req.body.entity.attributes.blog_type === 'finance':
      return EmailFinanceIta(
        process.env.PAKO_WEBSITE,
        req.body.entity,
        cover,
        user,
      )
